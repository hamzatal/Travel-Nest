...
import {
  MapPin, ArrowRight, MessageCircle, X, ArrowUp, Heart, Star,
  TrendingUp, Shield, Tag, Award, Compass, Image, Coffee, Globe,
  Briefcase, Gift, UserCheck, Sun,
  ArrowLeft
} from "lucide-react";
...
const HomePage = ({ auth }) => {
  const { props } = usePage();
  const { heroSections = [], flash } = props; // Default to empty array
  ...
  const trendingDestinations = [
    ...
    { name: "Santorini, Greece", image: "https://images.unsplash.com/photo-1602081593934-b723cf961d3e", price: "$1,200", rating: 4.9, discount: null, tag: "Luxury" },
  ];
  ...
  {heroSections.length === 0 ? (
    <div className="absolute inset-0 flex items-center justify-center bg-gradient-to-r from-gray-900 to-gray-800 text-white text-xl">
      No hero sections available. Please add some in the admin dashboard.
    </div>
  ) : (
    <>
        ...
        <AnimatePresence initial={false}>
          ...
          >
            <div className="relative w-full h-full">
              ...
              <img
                src={heroSections[currentSlide]?.image || "https://via.placeholder.com/1920x1080"}
                alt={heroSections[currentSlide]?.title || "Hero Image"}
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center px-6 z-10 pt-44">
        ...
        >
          <h1 className="text-5xl md:text-6xl lg:text-7xl font-bold mb-6 leading-tight text-white">
            {heroSections[currentSlide]?.title || "Welcome to Travel Nest"}
          </h1>
          
          <p className="text-xl md:text-2xl text-gray-100 mb-10 max-w-3xl mx-auto font-light">
            {heroSections[currentSlide]?.subtitle || "Plan your next adventure with us."}
          </p>
          
          <div className="flex flex-col sm:flex-row items-center justify-center gap-5">
            <button
              className="px-8 py-4 bg-white text-gray-900 rounded-full shadow-lg hover:bg-gray-100 transition-all duration-300 font-semibold text-lg min-w-48"
            >
              {heroSections[currentSlide]?.cta_text || "Start Planning"}
            </button>
            <button
      ...
      <div className="absolute inset-x-0 top-1/2 -translate-y-1/2 flex justify-between px-4 md:px-10 z-20">
        <button 
          onClick={() => setCurrentSlide((prev) => (prev === 0 ? heroSections.length - 1 : prev - 1))}
        ...
        <button 
          onClick={() => setCurrentSlide((prev) => (prev === heroSections.length - 1 ? 0 : prev + 1))}
          className="w-10 h-10 md:w-12 md:h-12 flex items-center justify-center rounded-full bg-white/20 backdrop-blur-sm text-white hover:bg-white/30 transition-all"
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between mb-10">
              ...
              <div className="flex mt-4 md:mt-0 gap-6">
                <button
                  ...
                  className={`px-4 py-2 rounded-full ${activeTab === "popular" ? "bg-blue-600 text-white" : isDarkMode ? "bg-gray-700 text-gray-300" : "bg-white text-gray-700"}`}
                >
                  Most Popular
                </button>
                <button
                  onClick={() => setActiveTab("deals")}
                  className={`px-4 py-2 rounded-full ${activeTab === "deals" ? "bg-blue-600 text-white" : isDarkMode ? "bg-gray-700 text-gray-300" : "bg-white text-gray-700"}`}
                >
                  Best Deals
                </button>
                <button
                  onClick={() => setActiveTab("hidden")}
                  className={`px-4 py-2 rounded-full ${activeTab === "hidden" ? "bg-blue-600 text-white" : isDarkMode ? "bg-gray-700 text-gray-300" : "bg-white text-gray-700"}`}
                >
                  Hidden Gems
                </button>
              </div>
                ...
                >
                  ...
                  <div className="p-5">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <button className={`flex items-center gap-1 text-sm ${isDarkMode ? "text-blue-400" : "text-blue-600"} hover:underline`} aria-label="View destination details">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {[
                ...
                >
                  ...
                  <h3 className={`text-lg font-semibold mb-2 ${isDarkMode ? "text-white" : "text-gray-900"}`}>{category.title}</h3>
        ...
        <div className="max-w-7xl mx-auto px-6 md:px-16">
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
              {[
                { icon: <Tag size={24} />, title: "Best Price Guarantee", description: "We match or beat any comparable price you can find elsewhere." },
                { icon: <Shield size={24} />, title: "Secure Booking", description: "Your personal and payment information is always protected." },
                { icon: <Award size={24} />, title: "Top-rated Service", description: "Our dedicated support team is available 24/7 to assist you." },
                { icon: <TrendingUp size={24} />, title: "Loyalty Rewards", description: "Earn points on every booking and unlock exclusive benefits." },
              ].map((benefit, index) => (
                <motion.div
                  key={index}
                  whileHover={{ y: -5 }}
